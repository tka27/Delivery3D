using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

public class BuyUpgradeInfo : MonoBehaviour
{
    [HideInInspector] public float[] perksPrices = new float[5];
    float[] perksPricesMultiplier = new float[] { 1, 1.2f, 1.4f, 1.6f, 1.8f };
    [SerializeField] StaticData staticData;
    [SerializeField] MainMenuSceneData mainMenuSceneData;
    [SerializeField] List<Text> lvls;
    [SerializeField] List<Text> prices;



    void Update()
    {
        if (mainMenuSceneData.upgradesUpdateRequest)
        {
            mainMenuSceneData.upgradesUpdateRequest = false;

            for (int i = 0; i < perksPrices.Length; i++)
            {
                int perkLvl = staticData.carPerks[staticData.selectedCarID][i];
                float carPrice = staticData.allCars[staticData.selectedCarID].price;
                perksPrices[i] = perksPricesMultiplier[i] * carPrice / 10 + perksPricesMultiplier[i] * carPrice / 10 * perkLvl;
                lvls[i].text = "lvl : " + perkLvl.ToString();
                prices[i].text = perksPrices[i].ToString("0.0");
            }


        }

    }
}
